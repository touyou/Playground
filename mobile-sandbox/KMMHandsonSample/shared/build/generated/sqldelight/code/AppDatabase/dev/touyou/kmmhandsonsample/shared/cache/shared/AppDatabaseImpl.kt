package dev.touyou.kmmhandsonsample.shared.cache.shared
...
import dev.touyou.kmmhandsonsample.shared.cache.AppDatabase
import dev.touyou.kmmhandsonsample.shared.cache.AppDatabaseQueries
import dev.touyou.kmmhandsonsample.shared.cache.Rocket
import dev.touyou.kmmhandsonsample.shared.cache.SelectAllLaunchesInfo
import kotlin.Any
...
private class AppDatabaseQueriesImpl(
  ...
  override fun <T : Any> selectAllLaunchesInfo(mapper: (
    ...
    name: String?,
    type: String?
  ) -> T): Query<T> = Query(-698618385, selectAllLaunchesInfo, driver, "AppDatabase.sq",
  ...
  override fun insertLaunch(
    ...
    notifyQueries(-1221356783, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  override fun insertRocket(
    ...
    notifyQueries(-1037191630, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllLaunches() {
    driver.execute(1974928665, """DELETE FROM Launch""", 0)
    notifyQueries(1974928665, {database.appDatabaseQueries.selectAllLaunchesInfo})
  }

  override fun removeAllRockets() {
    driver.execute(-1293086809, """DELETE FROM Rocket""", 0)
    notifyQueries(-1293086809, {database.appDatabaseQueries.selectRocketById +
        database.appDatabaseQueries.selectAllLaunchesInfo})
  }
  ...
  private inner class SelectRocketByIdQuery<out T : Any>(
    ...
    override fun execute(): SqlCursor = driver.executeQuery(-238692441, """
    |SELECT * FROM Rocket
    |WHERE id = ?
    """.trimMargin(), 1) {
      bindString(1, id)
